...
class sim:
    '''
    Simulates COVID in a population using a SIR-style model.  Infections last a
    single generation, after which people recover and are immune for the
    remainder of the simulation.  Infections may be discovered immediately
    (e.g., through symptomatic self-reporting or surveillance) or may be
    discovered later (e.g., because of a 1-time asymptomatic test applied to
    the whole population, or because the student had symptoms due to another
    cause and got a COVID test).

    Supports multiple groups. Takes as input a matrix (infection_matrix) that
    determines the number of secondary infections created in each group j over
    1 generation by a single infection in group i.

    Appropriate choices for this matrix can be determined using the micro
    class.  This takes as input testing interventions as well as social and
    biological parameters to produce this matrix.

    During execution, S, I, and R are TxK matrices that track the number
    susceptible, infectious, and recovered in each of the K groups over T time
    periods.
    '''
    def __init__(self, max_T: int, init_susceptible: np.ndarray,
        ...
        Group dynamics can be captured by providing vectors of initial counts.
        If there are K groups, the length of init_susceptible, init_infected,
        and init_recovered should all be K.

        To model the discovery of people who have or had COVID (I or R), we
        model a fraction infection_discovery_frac of new infections as being
        discovered in the generation that they start. If they are not
        discovered then, they become "hidden recovered".  A fraction
        recovered_discovery_frac of the undiscovered recovered infections are
        ...
        self.D[0] = np.multiply(self.I[0],self.infection_discovery_frac)
    ...
    def get_metric_for_different_groups(self, metric, group, normalize=False,
                                        cumulative=False):
        '''
        Returns a matrix where [t, i] entry contains the number of people in
        group[i] of a particular type (S,I,R,D,H) in generation t. If normalize
        is true, then this is normalized to the group's population size.  For
        example, to get a vector containing the number of people with an active
        infection in groups [0, 1] and each generation, call
        get_metric_for_different_groups('I', [0, 1]).
        '''
        assert(len(group) > 0)
        assert(min(group) >= 0)
        ...
        if normalize:
            # total population by time
            pop = self.S[:, group] + self.I[:, group] + self.R[:, group]
            y = y / pop
    ...
    def get_metric_for_group(self, metric, group_idxs, normalize=False,
                             cumulative=False):
        '''
        Returns a vector where component t contains the number of people of a
        particular type (S,I,R,D,H) in generation t in a specific set of group.
        If normalize is true, then this is normalized to the group's population
        size.  For example, to get a vector containing the number of people
        with an active infection in group 0 and each generation, call
        get_metric_in_group('I').
        '''
    ...
    def get_metric(self, metric, aggregate=True, normalize=False, cumulative=False):
        '''
        Returns a vector where component t contains the total number of people
        of a particular type (S,I,R,D,H) in generation t.  If aggregate is
        true, then this is summed across the groups.  If normalize is true,
        then this is normalized to the population size.  When normalize = True
        and aggregate = False, then the fraction returned is the number of
        people in the group infected divided by the size *of that group* For
        example, to get a vector containing the total number of people that
        were discovered each week, call get_metric('D'). To make it cumulative,
        call get_metric('D',cumulative=True)
        '''
    ...
    def get_infected_for_group(self, metric, group, normalize=False,
                               cumulative=False):
        return self.get_metric_for_group('I', group, normalize, cumulative)

    def get_total_infected_for_different_groups(self, group, normalize=False,
                                                cumulative=False):
        """
    ...
    def get_total_discovered_for_different_groups(self, group, normalize=False,
                                                  cumulative=False):
        """
    ...
    def get_isolated(self, group=False, iso_lengths=[8], iso_props=[1]):
        ...
        iso_lengths is the number days isolations lasts for each group (in
        ascending order) group is the group to look at.  If group is false,
        then aggregate across everyone.  If group is specified as a list of
        group indices, then it aggregates across these group indices.

        iso_props is the proportion of people in each isolation length group,
        so len(iso_lengths) and len(iso_props) must be the same.
        ...
        isolation_frac[i] is the fraction of people discovered i generations
        ago that require isolation in the current generation. For example,
        suppose 80% of people require isolation for 2 generations and 20% for
        only 1. Then isolation_frac = [1, .2] is appropriate because all of the
        people discovered in the current generation require isolation and only
        20% of those discovered in the previous generation still require
        isolation.

        Also, if the people discovered in a particular generation (say, i
        generations ago) only need isolation for a fraction of the current
        generation, then it is appropriate to set isolation_frac[i] to that
        fraction.  For example, if isolation lasts 10 days for all individuals
        and the generation time is 4 days, we can set isolation = 3,
        isolation_frac = [1, 1, .5], where isolation_frac[2] = 0.5 because the
        positives discovered 2 generations ago only need isolation for 10 - 2*4
        = 2 days out of 4 in the current generation.
